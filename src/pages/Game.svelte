<script>
    ...
    async function startGame() {
        const res = await fetch(`http://localhost:8002/user/${username}/start-game`, {
            method: "POST",
        });
    ...
    async function submitGuess() {
        ...
        const data = await res.json();
        if (res.ok) {
            resultMessage = data.correct ? "🎉 Correct! It's Paris!" : "❌ Wrong guess!";
        } else {
...
<main class="p-6 max-w-lg mx-auto">
    <h1 class="text-4xl text-center font-bold mb-10">GeoGuessr Game</h1>
    ...
    {:else}
        {#if !gameStarted}
            <button
            class="w-full bg-blue-600 text-white px-4 py-2 rounded mb-4 cursor-pointer hover:bg-blue-700 transition"
            on:click={startGame}
            >
            Start Game
            </button>
        ...
        {#if gameStarted}
            <div class="space-y-4">
                ...
                <div class="flex justify-center">
                    ...
                    alt="Map" class="h-full w-50 object-cover rounded" />
                </div>
        ...
        {#if resultMessage}
            <p class="mt-4 text-lg font-semibold">{resultMessage}</p>
        {/if}
    {/if}
</main>
